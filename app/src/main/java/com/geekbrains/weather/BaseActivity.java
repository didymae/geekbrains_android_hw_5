...
public class BaseActivity extends AppCompatActivity
    ...
    private void initLayout() {
        ...
        // Если устройство перевернули в альбомную ориентацию, то надо показать два фрагмента
            addFragmentSet(new WeatherFragment(),new CreateActionFragment());
        } else addFragment(new WeatherFragment());
    }
    ...
    private void addFragmentSet(Fragment fragment1,Fragment fragment2) {
        //вызываем SupportFragmentManager и указываем в каком контейнере будет находиться наш фрагмент
        getSupportFragmentManager()
                .beginTransaction()
                .add(R.id.content_frame1, fragment1)
                .add(R.id.content_frame2, fragment2)
                .commit();
    }
    ...
    public void startWeatherFragment(String country) {
        ...
        if (getResources().getConfiguration().orientation ==
                ORIENTATION_LANDSCAPE) {
            addFragmentSet(WeatherFragment.newInstance(country),new CreateActionFragment());
        }
                else
